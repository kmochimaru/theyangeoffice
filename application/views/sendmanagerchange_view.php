                    ...
                    <div class="row">
                        <div class=" col-md-12 text-left">
                            ...
                            <select id="def_id_pri" name="def_id_pri_select[]" multiple="multiple">
                                <?php
                                $orgs = $this->managerlist_model->getOrganization();
                                if ($orgs->num_rows() > 0) {
                                    foreach ($orgs->result() as $org) {
                                        $deps = $this->managerlist_model->getOrgDepartment($org->org_id_pri);
                                        if ($deps->num_rows() > 0) {
                                            foreach ($deps->result() as $dep) {
                                                $dep_offs = $this->managerlist_model->getdep_off($dep->dep_id_pri);
...
<script>
    ...
    $('#from_add').submit(function(e) {
        ...
        $.ajax({
            url: '<?php echo base_url() . 'managerlist/changesendto'; ?>',
            type: 'POST',
            data: postData,
            processData: false,
            contentType: false,
