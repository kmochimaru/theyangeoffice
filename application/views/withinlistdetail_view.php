      ...
      <div class="card-body">
        <h4 class="card-title">
          ...
          <span style="float: right">
            <a href="<?php echo base_url() . 'withinlist'; ?>" class="btn btn-sm btn-outline-secondary"><i class="fa fa-arrow-left"></i> กลับ</a>
          </span>
        </h4>
        <hr>
        <div class="row">
          <div class="col-md-12 col-sm-12 text-left">
            <div class="row">
              ...
              <div class="col-sm-10 m-t-0">
                <?php
                $files = $this->withinlist_model->get_workinfofile($data->work_info_id_pri);
                ?>
        ...
        <div class="row">
          <div class="col-md-12 text-right">
            <?php if ($data->state_info_id <= 4) { ?>
              <a href="<?php echo base_url() . 'withinlist/attach/' . $data->work_info_code; ?>" class="btn btn-info"><i class="mdi mdi-paperclip"></i>&nbsp;เพิ่มไฟล์แนบ</a>
            <?php } ?>
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                    ...
                    foreach ($files->result() as $file) { ?>
                      <tr>
                        ...
                        <td><a style="padding-right: 5px" href="<?php echo base_url() . 'store/file/' . $file->work_info_file_id; ?>" title="<?php echo $file->work_info_file_oldname; ?>" class="<?php echo ($file->file_type_check != 2) ? ($file->file_type_check != 1) ? 'fancybox' : 'fancyboxpdf' : ''; ?>" target="_blank">
                            <?php echo $file->work_info_file_oldname; ?></a></td>
                        <td colspan="2">ต้นเรื่อง</td>
                        <td><?php echo $this->withinlist_model->get_user($file->user_id)->user_fullname; ?></td>
                  ...
                  $workprocessfiles = $this->withinlist_model->get_workprocessfofile($data->work_info_id_pri);
                    ...
                    ?>
                      <tr>
                        ...
                        <td><a style="padding-right: 5px" href="<?php echo base_url() . 'store/workprocessfile/' . $workprocessfile->work_process_file_id; ?>" title="Item Title" class="<?php echo ($workprocessfile->file_type_check != 2) ? ($workprocessfile->file_type_check != 1) ? 'fancybox' : 'fancyboxpdf' : ''; ?>" target="_blank">
                            <?php echo $workprocessfile->work_process_file_oldname; ?></a></td>
                        <?php
                        if ($workprocessfile->work_process_file_send == 1) {
                          echo '<td colspan="2">ต้นเรื่อง (เอกสารเพิ่มเติม)</td>';
                        } else {
                          $dep_off = $this->withinlist_model->getdep_off_id($workprocessfile->dep_off_id);
                          echo '<td>' . $dep_off->dep_name . '</td><td>' . $dep_off->officer_name_display . '</td>';
                        }
                        ?>
                        <td><?php echo $this->withinlist_model->get_user($workprocessfile->user_id)->user_fullname; ?></td>
                  ...
                  $workuserfofiles = $this->withinlist_model->get_workuserfofile($data->work_info_id_pri);
                  if ($workuserfofiles->num_rows() > 0) {
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <?php
                          $file_work_process = $this->withinlist_model->get_workuserid($workuserfofile->work_user_id)->row();
                          $dep_off = $this->withinlist_model->getdep_off_id($file_work_process->dep_off_id);
                          echo $dep_off->dep_name;
                          ?>
                        </td>
                        <td><?php echo 'ผู้ปฏิบัติงาน'; ?></td>
                        <td><?php echo $this->withinlist_model->get_user($workuserfofile->user_id)->user_fullname; ?></td>
          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tbody>
                <?php
                $i = 1;
                $rows = $this->withinlist_model->get_withinprocess($work_info_id_pri);
                if ($rows->num_rows() > 0) {
                ...
                ?>
                    <tr>
                      ...
                      <td style="font-weight: bold;">
                        ...
                        <?php } ?>
                        <?php
                        $checks = $this->withinlist_model->checkwithinprocess($row->work_info_id_pri, ($row->work_process_sort - 1), 0);
                      ...
                      <td style="border-right: 1px solid whitesmoke;">
                        ...
                        if ($row->work_process_id_to == null) {
                          if ($row->work_user_id != null) {
                            echo  $this->withinlist_model->get_user($row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )';
                          } else {
                            echo $this->withinlist_model->getdep_off_id($data->dep_off_id)->dep_name . '<br> ( ' . $this->withinlist_model->getdep_off_id($data->dep_off_id)->officer_name_display . ' )';
                          }
                        } else {
                          $dep_off_to = $this->withinlist_model->getdep_off_id($this->withinlist_model->getworkprocess($row->work_process_id_to)->row()->dep_off_id);
                      ...
                      <td>
                        <?php
                        $dep_off_to = $this->withinlist_model->getdep_off_id($row->dep_off_id);
                    ...
                    <tr>
                      ...
                      <?php $special_command_name = $this->withinlist_model->ref_special_command($row->special_command_id)->row()->special_command_name; ?>
                    ...
                    <?php
                    $i++;
                    $user_rows = $this->withinlist_model->get_workuser($row->work_process_id_pri);
                    ...
                    ?>
                        <tr>
                          ...
                          <td style="border-right: 1px solid whitesmoke;">
                            <?php
                            $dep_off_id = $this->withinlist_model->getdep_off_id($this->withinlist_model->getworkprocess($row->work_process_id_pri)->row()->dep_off_id);
                            echo $dep_off_id->dep_name . '<br> ( ' . $dep_off_id->officer_name_display . ' )';
                            ?>
                          </td>
                          <td><?php echo  $this->withinlist_model->get_user($user_row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )'; ?></td>
                          <td class="text-center"><?php echo '-'; ?></td>
                          <?php if ($user_row->work_user_status_id != 1) { ?>
                            <td><?php echo $this->withinlist_model->get_user($user_row->user_id)->user_fullname; ?></td>
...
<script>
  ...
  function modal_closestatus(work_info_id_pri) {
    $.ajax({
      url: service_base_url + 'withinlist/modal_closestatus',
      type: 'POST',
      data: {
  ...
  function closestatus(work_info_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'withinlist/closestatus',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
  ...
  function follow(work_info_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'withinlist/follow',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
      success: function(response) {
        if (response == 1) {
          ...
          setTimeout(function() {
            window.location.href = '<?php echo base_url() . 'withinlist/detail/' . $data->work_info_code; ?>';
          }, 2500);
        } else {
  ...
  function unfollow(work_info_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'withinlist/unfollow',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
