<form method="post" action="<?php echo base_url('user/status_user'); ?>" autocomplete="off">
    ...
    <input type="hidden" name="user_status_id" value="<?php echo $data->user_status_id == 1 ? 2 : 1; ?>">
    <div class="modal-header">
        <h4 class="modal-title"><i class="fa fa-close"></i> <?php echo $data->user_status_id == 1 ? 'ยืนยันการระงับ' : 'ยืนยันยกเลิกการระงับ'; ?>ผู้ใช้ระบบ</h4>
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    </div>
    <div class="modal-body">
        <div class="bootbox-body text-center text-danger">
            <b><?php echo $data->user_status_id == 1 ? 'ยืนยันการระงับ' : 'ยืนยันยกเลิกการระงับ'; ?>ผู้ใช้ระบบนี้ ใช่หรือไม่ ?</b>
        </div>
    </div>
