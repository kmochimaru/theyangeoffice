        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="row" style="margin-left: -5px;">
                    <div class=" col-md-12">
                        <?php
                        $groupdep = $this->withinwaiting_model->get_groupdep();
                        ...
                        <div class=" col-md-12 text-left">
                            ...
                            <div id="result-pagination">
                                <!-- <select id="def_id_pri" name="def_id_pri_select[]" multiple="multiple">
                                    <?php
                                    // $orgs = $this->withinwaiting_model->getOrganization();
                                    // if ($orgs->num_rows() > 0) {
                                    //     foreach ($orgs->result() as $org) {
                                    //         $deps = $this->withinwaiting_model->getOrgDepartment($org->org_id_pri);
                                    //         if ($deps->num_rows() > 0) {
                                    //             foreach ($deps->result() as $dep) {
                                    //                 $dep_offs = $this->withinwaiting_model->getDepOffSend($dep->dep_id_pri);
...
<script>
    ...
    $('#from_add').submit(function(e) {
        ...
        if ($('#work_process_sendtype').val() == 1) {
            url = '<?php echo base_url() . 'withinwaiting/sendto'; ?>'
        } else {
            url = '<?php echo base_url() . 'withinwaiting/sendtosort'; ?>'
        }
        $.ajax({
            url: url,
            type: 'POST',
            ...
            success: function(response) {
                ...
                if (response == 0) {        
                    ...
                    window.location.href = '<?php echo base_url() . 'withinprocess'; ?>';
                }
            }
        })
    })
    ...
    function follow() {
        ...
        $.ajax({
            url: service_base_url + 'withinwaiting/follow',
            type: 'POST',
            data: {
    ...
    function attach_original(attach_original) {
        $.ajax({
            url: service_base_url + 'withinwaiting/attach_original',
            type: 'POST',
            data: {
                work_info_id_pri: $('#work_info_id_pri').val(),
