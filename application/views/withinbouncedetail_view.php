      ...
      <div class="card-body">
        <h4 class="card-title">
          ...
          <span style="float: right" class="text-muted">
            <a href="<?php echo base_url() . 'withinbounce'; ?>" class="btn btn-sm btn-outline-secondary"><i class="fa fa-arrow-left"></i> กลับ</a>
          </span>
        </h4>
        <hr>
        <div class="row">
          <div class="col-md-12 text-left">
            <div class="form-group row">
              ...
              <div class="col-sm-10 m-t-0">
                <?php
                $files = $this->withinbounce_model->get_workinfofile($data->work_info_id_pri);
                ?>
        ...
        <?php if ($this->withinbounce_model->checkworkprocess($work_process_id_pri)->row()->work_process_receive == 0) { ?>
          <hr>
          <div class="row">
        ...
        <?php } else { ?>
          ...
          <div class="row">
            <div class="col-md-12 text-right">
              ...
              <!-- <a href="<?php echo base_url() . 'withinbounce/attachall/' . $data->work_info_code . '/' . $work_process_id_pri; ?>" class="btn btn-info"><i class="mdi mdi-paperclip"></i>&nbsp;เพิ่มไฟล์แนบ</a>
                            <a href="<?php echo base_url() . 'withinbounce/attach/' . $work_process_id_pri; ?>" class="btn btn-outline-primary"><i class="fa fa-paperclip"></i>&nbsp;เพิ่มไฟล์เอกสาร</a> -->
                ...
                <tbody>
                  ...
                  ?>
                      <tr>
                        ...
                        <td><?php echo $this->withinbounce_model->get_user($file->user_id)->user_fullname; ?></td>
                  ...
                  $workprocessfiles = $this->withinbounce_model->get_workprocessfofile($this->withinbounce_model->checkworkprocess($work_process_id_pri)->row()->work_process_id_to);
                    ...
                    ?>
                      <tr>
                        ...
                        if ($workprocessfile->work_process_file_send == 1) {
                          ...
                          $dep_off = $this->withinbounce_model->getdep_off_id($workprocessfile->dep_off_id);
                          echo '<td>' . $dep_off->dep_name . '</td><td>' . $dep_off->officer_name_display . '</td>';
                        }
                        ?>
                        <td><?php echo $this->withinbounce_model->get_user($workprocessfile->user_id)->user_fullname; ?></td>
...
<?php if ($this->withinbounce_model->checkworkprocess($work_process_id_pri)->row()->work_process_receive == 1) { ?>
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  <?php
                  $i = 1;
                  $rows = $this->withinbounce_model->get_withinprocess($data->work_info_id_pri);
                  if ($rows->num_rows() > 0) {
                  ...
                  ?>
                      <tr>
                        ...
                        <td style="font-weight: bold;">
                          ...
                          <?php } ?>
                          <?php
                          $checks = $this->withinbounce_model->checkwithinprocess($row->work_info_id_pri, ($row->work_process_sort - 1), 0);
                        ...
                        <td style="border-right: 1px solid whitesmoke;">
                          ...
                          if ($row->work_process_id_to == null) {
                            if ($row->work_user_id != null) {
                              echo  $this->withinbounce_model->get_user($row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )';
                            } else {
                              echo $this->withinbounce_model->getdep_off_id($data->dep_off_id)->dep_name . '<br> ( ' . $this->withinbounce_model->getdep_off_id($data->dep_off_id)->officer_name_display . ' )';
                            }
                          } else {
                            $dep_off_to = $this->withinbounce_model->getdep_off_id($this->withinbounce_model->getworkprocess($row->work_process_id_to)->row()->dep_off_id);
                        ...
                        <td>
                          <?php
                          $dep_off_to = $this->withinbounce_model->getdep_off_id($row->dep_off_id);
                      ...
                      <tr>
                        ...
                        <?php $special_command_name = $this->withinbounce_model->ref_special_command($row->special_command_id)->row()->special_command_name; ?>
                      ...
                      <?php
                      $i++;
                      $user_rows = $this->withinbounce_model->get_workuser($row->work_process_id_pri);
                      ...
                      ?>
                          <tr>
                            ...
                            <td style="border-right: 1px solid whitesmoke;">
                              <?php
                              $dep_off_id = $this->withinbounce_model->getdep_off_id($this->withinbounce_model->getworkprocess($row->work_process_id_pri)->row()->dep_off_id);
                              echo $dep_off_id->dep_name . '<br> ( ' . $dep_off_id->officer_name_display . ' )';
                              ?>
                            </td>
                            <td><?php echo  $this->withinbounce_model->get_user($user_row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )'; ?></td>
                            <td class="text-center"><?php echo '-'; ?></td>
                            <?php if ($user_row->work_user_status_id != 1) { ?>
                              <td><?php echo $this->withinbounce_model->get_user($user_row->user_id)->user_fullname; ?></td>
...
<div id="modal_comment" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form id="form_modal_comment" method="post" action="<?php echo base_url('withinbounce/add_comment'); ?>" enctype="multipart/form-data" autocomplete="off">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <textarea name="work_process_receive_comment" id="work_process_receive_comment" class="form-control form-control-sm" rows="4" required=""><?php echo $this->withinbounce_model->getworkprocess($work_process_id_pri)->row()->work_process_receive_comment ?></textarea>
          </div>
        </div>
        <div class="modal-footer">
...
<div id="modal_back_2" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form id="form_modal_back_2" method="post" action="<?php echo base_url('withinbounce/add_back_2'); ?>" enctype="multipart/form-data" autocomplete="off">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input type="hidden" name="work_process_id_pri" value="<?php echo $work_process_id_pri; ?>">
            <textarea name="work_process_receive_comment_2" id="work_process_receive_comment_2" class="form-control form-control-sm" rows="4" required=""><?php echo $this->withinbounce_model->getworkprocess($work_process_id_pri)->row()->work_process_receive_comment ?></textarea>
          </div>
        </div>
        <div class="modal-footer">
...
<script>
  ...
  function modal_receive() {
    $.ajax({
      url: service_base_url + 'withinbounce/modal_receive',
      type: 'POST',
      data: {
  ...
  function sendback() {
    ...
    $.ajax({
      url: service_base_url + 'withinbounce/sendback',
      type: 'POST',
      data: {
      ...
      success: function(response) {
        ...
        if (response == 1) {
          window.location.href = '<?php echo base_url() . 'withinbounce'; ?>';
        } else {
  ...
  function modal_sendstatus() {
    $.ajax({
      url: service_base_url + 'withinbounce/modal_sendstatus',
      type: 'POST',
      data: {
        work_process_id_pri: $('#work_process_id_pri').val(),
      },
  ...
  function withinbounce(work_process_id_pri) {
    $('#btn_submit').prop('disabled', true);
    $('#btn-modal-submit-loading').show();
    work_process_sendtype = $('#work_process_sendtype').val();
    $.ajax({
      url: service_base_url + 'withinbounce/receive',
      type: 'POST',
      data: {
      ...
      success: function(response) {
        $('#result-modal').modal('hide');
        if (response == 1) {
          window.location.href = '<?php echo base_url() . 'withinbounce/detail/'; ?>' + work_process_id_pri;
        } else {
  ...
  function receivesend(work_process_id_pri) {
    ...
    $('#btn-modal-submit-loading').show();
    $.ajax({
      url: service_base_url + 'withinbounce/receivesend',
      type: 'POST',
      data: {
        work_process_id_pri: work_process_id_pri,
      },
  ...
  function modal_closestatus(work_info_id_pri) {
    $.ajax({
      url: service_base_url + 'withinbounce/modal_closestatus',
      type: 'POST',
      data: {
  ...
  function closestatus(work_info_id_pri) {
    ...
    $('#btn-modal-submit-loading').show();
    $.ajax({
      url: service_base_url + 'withinbounce/closestatus',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
