      ...
      <div class="card-body">
        <h4 class="card-title">
          ...
          <span style="float: right">
            <a href="<?php echo base_url() . 'actinglist'; ?>" class="btn btn-sm btn-outline-secondary"><i class="fa fa-arrow-left"></i> กลับ</a>
          </span>
        </h4>
        <hr>
        <div class="row">
          <div class="col-md-12 col-sm-12 text-left">
            <div class="row">
              ...
              <div class="col-sm-10 m-t-0">
                <?php
                $files = $this->actinglist_model->get_workinfofile($data->work_info_id_pri);
                ?>
        ...
        <div class="row">
          <div class="col-md-12 text-right">
            <?php if ($data->state_info_id <= 4) { ?>
              <a href="<?php echo base_url() . 'actinglist/attach/' . $data->work_info_code; ?>" class="btn btn-info"><i class="mdi mdi-paperclip"></i>&nbsp;เพิ่มไฟล์แนบ</a>
            <?php } ?>
            <?php if ($data->state_info_id == 5) { ?>
              <a href="javascript:void(0)" onclick="modal_publicstatus('<?php echo $data->work_info_id_pri; ?>');" class="btn btn-primary"><i class="fa fa fa-bullhorn"></i> ประกาศรักษาการ</a>
            <?php } ?>
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  ...
                  if ($files->num_rows() > 0) {
                    foreach ($files->result() as $file) { ?>
                      <tr>
                        ...
                        <td><?php echo $this->actinglist_model->get_user($file->user_id)->user_fullname; ?></td>
                  ...
                  $workprocessfiles = $this->actinglist_model->get_workprocessfofile($data->work_info_id_pri);
                    ...
                    ?>
                      <tr>
                        ...
                        if ($workprocessfile->work_process_file_send == 1) {
                          ...
                          $dep_off = $this->actinglist_model->getdep_off_id($workprocessfile->dep_off_id);
                          echo '<td>' . $dep_off->dep_name . '</td><td>' . $dep_off->officer_name_display . '</td>';
                        }
                        ?>
                        <td><?php echo $this->actinglist_model->get_user($workprocessfile->user_id)->user_fullname; ?></td>
                  ...
                  $workuserfofiles = $this->actinglist_model->get_workuserfofile($data->work_info_id_pri);
                  if ($workuserfofiles->num_rows() > 0) {
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <?php
                          $file_work_process = $this->actinglist_model->get_workuserid($workuserfofile->work_user_id)->row();
                          $dep_off = $this->actinglist_model->getdep_off_id($file_work_process->dep_off_id);
                          echo $dep_off->dep_name;
                          ?>
                        </td>
                        <td><?php echo 'ผู้ปฏิบัติงาน'; ?></td>
                        <td><?php echo $this->actinglist_model->get_user($workuserfofile->user_id)->user_fullname; ?></td>
          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tbody>
                <?php
                $i = 1;
                $rows = $this->actinglist_model->get_workinprocess($work_info_id_pri);
                if ($rows->num_rows() > 0) {
                ...
                ?>
                    <tr>
                      ...
                      <td style="font-weight: bold;">
                        ...
                        <?php } ?>
                        <?php
                        $checks = $this->actinglist_model->checkworkinprocess($row->work_info_id_pri, ($row->work_process_sort - 1), 0);
                      ...
                      <td>
                        <?php if ($row->work_process_id_to == null) { ?>
                          <span><?php echo $this->actinglist_model->getdep_off_id($row->dep_off_id)->dep_name; ?></span>
                        <?php
                        } else {
                          $dep_off_to = $this->actinglist_model->getdep_off_id($this->actinglist_model->getworkprocess($row->work_process_id_to)->row()->dep_off_id);
                        ?>
                          <span><?php echo $this->actinglist_model->getdep_off_id($row->dep_off_id)->dep_name; ?></span>
                          <br>
                      ...
                      <td><?php echo $this->actinglist_model->getdep_off_id($row->dep_off_id)->officer_name_display; ?></td>
                      <?php if ($row->work_process_receive == 1) { ?>
                    ...
                    <?php
                    $i++;
                    $user_rows = $this->actinglist_model->get_workuser($row->work_process_id_pri);
                    ...
                    ?>
                        <tr>
                          ...
                          <td>
                            <?php
                            echo $this->actinglist_model->get_user($user_row->user_id)->user_fullname;
                            $dep_off_id = $this->actinglist_model->getdep_off_id($this->actinglist_model->getworkprocess($row->work_process_id_pri)->row()->dep_off_id);
                            ?>
                            <br>
                          ...
                          <?php if ($user_row->work_user_status_id != 1) { ?>
                            <td><?php echo $this->actinglist_model->get_user($user_row->user_id)->user_fullname; ?></td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="bootbox-body text-center">
          <p class="text-center" style="font-weight: bold;">เอกสารนี้ไม่สามารถประกาศรักษาการได้</p>
        </div>
      </div>
...
<script>
  ...
  function modal_publicstatus(work_info_id_pri) {
    $.ajax({
      url: service_base_url + 'actinglist/modal_publicstatus',
      type: 'POST',
      data: {
  ...
  function publicstatus(work_info_id_pri) {
    $('#btn_submit').prop('disabled', true);
    $('#btn-modal-submit-loading').show();
    $.ajax({
      url: service_base_url + 'actinglist/publicstatus',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
      success: function(response) {
        ...
        if (response == 1) {
          $('#result-modal').modal('hide');
          notification('success', 'สำเร็จ', 'ประกาศรักษาการสำเร็จ');
          setTimeout(function() {
            location.reload();
          }, 2500);
        } else {
          $('#result-modal').modal('hide');
          notification('error', 'ไม่สำเร็จ', 'ประกาศรักษาการไม่สำเร็จ!');
        }
      }
    })
  }
  ...
  function follow(work_info_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'actinglist/follow',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
      success: function(response) {
        if (response == 1) {
          ...
          setTimeout(function() {
            window.location.href = '<?php echo base_url() . 'actinglist/detail/' . $data->work_info_code; ?>';
          }, 2500);
        } else {
  ...
  function unfollow(work_info_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'actinglist/unfollow',
      type: 'POST',
      data: {
        work_info_id_pri: work_info_id_pri,
      },
