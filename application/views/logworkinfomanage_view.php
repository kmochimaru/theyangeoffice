...
<div id="result_modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
...
<script>
    ...
    function getPagination() {
        ...
        $.ajax({
            url: service_base_url + 'logworkinfomanage/getpagination',
            type: 'POST',
            data: {
                start: $('#start').val(),
                end: $('#end').val(),
    ...
    function showLogDetail(log_id) {
        $('#modal_content').html('');
        $.ajax({
            url: service_base_url + 'logworkinfomanage/getlogdetail',
            type: 'POST',
            data: {
                log_id: log_id
            },
            success: function(response) {
                $('#result_modal #modal_content').html(response);
