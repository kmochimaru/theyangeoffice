<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">
                    <i class="fa fa-paperclip"></i>&nbsp;<?php echo 'เพิ่มไฟล์ - เลขที่เอกสาร' . ' '; ?><span class="text-muted"><?php echo $data->work_info_no . $data->work_info_no_2 . $data->work_info_no_3 . ' ( ส่งหน่วยงาน ' . $this->managerlist_model->getdep_off_id($workprocess->row()->dep_off_id)->officer_name . ' : ' . $this->managerlist_model->getdep_off_id($workprocess->row()->dep_off_id)->dep_name . ' )'; ?></span>
                ...
                <div id="close_upload">
                    ...
                    <div class=" col-md-12 text-right">
                        <a href="<?php echo base_url() . 'managerlist/detail/' . $work_process_id_pri; ?>" id class="btn btn-warning"><i class="fa fa-check-circle-o"></i>&nbsp;อัพโหลดเสร็จสิ้น</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12 text-right">
                        <a href="<?php echo base_url() . 'managerlist'; ?>" class="btn btn-secondary" ><i class="fa fa-arrow-left"></i>&nbsp;กลับ</a>
                        <a href="<?php echo base_url() . 'managerlist/detail/' . $work_process_id_pri; ?>" class="btn btn-info" ><i class="fa fa-navicon"></i>&nbsp;รายละเอียด</a>                        
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function upload_attach() {
        ...
        url = service_base_url + 'managerlist/upload_attach/' + $('#work_process_id_pri').val();
        $.ajax({
            url: url,
            type: 'POST',
            contentType: false,
    ...
    function ajax_page() {
        ...
        $('#result_page').html('<div style="text-align:center; padding:80px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');
        $.ajax({
            url: service_base_url + 'managerlist/ajax_page',
            type: 'post',
            data: {
    ...
    function delete_file(id, name) {
        url = service_base_url + 'managerlist/delete_file';
        $.ajax({
            url: url,
            method: 'POST',
            data: {
