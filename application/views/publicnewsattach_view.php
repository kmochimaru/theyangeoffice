            ...
            <div class="card-body">
                <h4 class="card-title">
                    ...
                    <span style="float: right; margin-right: 5px;">
                        <a href="<?php echo base_url() . 'publicnewslist/notupload_attach/' . $data->work_info_id_pri; ?>" class="btn btn-sm btn-rounded btn-danger">
                ...
                <div class=" col-md-12 text-right">
                    <a href="<?php echo base_url() . 'publicnewslist/edit/' . $data->work_info_id_pri; ?>" class="btn btn-info" class="dropdown-item small"><i class="fa fa-edit"></i> แก้ไขเอกสาร</a>                  
                    <a href="javascript:void(0)" onclick="modal_regisnumber('<?php echo $data->work_info_id_pri; ?>');" class="btn <?php echo ($data->work_info_id != '' ? 'btn-success' : 'btn-primary'); ?>"><i class="fa fa-bullhorn"></i> ประกาศประชาสัมพันธ์</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12 text-right">
                        <a href="<?php echo base_url() . 'publicnewslist'; ?>" class="btn btn-secondary" ><i class="fa fa-arrow-left"></i>&nbsp;กลับ</a>                        
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="bootbox-body text-center">
                    <p class="text-center" style="font-weight: bold;">เอกสารนี้มีการประกาศประชาสัมพันธ์แล้ว</p>
                </div>
            </div>
...
<div id="modal_regisnumber_attach" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="bootbox-body text-center">
                    <p class="text-center" style="font-weight: bold;">กรุณาแนบไฟล์เอกสารก่อนประกาศประชาสัมพันธ์</p>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    function upload_attach() {
        ...
        url = service_base_url + 'publicnewslist/upload_attach/' + $('#work_info_id_pri').val();
        $.ajax({
            url: url,
            type: 'POST',
            contentType: false,
    ...
    function ajax_page() {
        ...
        $('#result_page').html('<div style="text-align:center; padding:80px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');
        $.ajax({
            url: service_base_url + 'publicnewslist/ajax_page',
            type: 'post',
            data: {
    ...
    function delete_file(id, name) {
        url = service_base_url + 'publicnewslist/delete_file';
        $.ajax({
            url: url,
            method: 'POST',
            data: {
    ...
    function modal_regisnumber(work_info_id_pri) {
        $.ajax({
            url: service_base_url + 'publicnewslist/modal_regisnumber',
            type: 'POST',
            data: {
            ...
            success: function (response) {
                if (response != 0) {
                    if (response != 1) {
                        ...
                        $('#modal_regisnumber_attach').modal('show', {backdrop: 'true'});
                    }
                } else {
                    $('#result-modal').modal('hide');
                    $('#modal_regisnumber_fail').modal('show', {backdrop: 'true'});
                }
            }
        });
    }
    ...
    function regisnumber(work_info_id_pri) {
        ...
        $.ajax({
            url: service_base_url + 'publicnewslist/regisnumber',
            type: 'POST',
            data: {
                work_info_id_pri: work_info_id_pri,
            },
            success: function (response) {
                $('#result-modal').modal('hide');
                if (response == 1) {
                    notification('success', 'สำเร็จ', 'ประกาศประชาสัมพันธ์สำเร็จ');
                    window.location.href = service_base_url + 'publicnewslist';
                } else {
                    notification('error', 'ไม่สำเร็จ', 'ประกาศประชาสัมพันธ์ไม่สำเร็จ!');
                    window.location.href = service_base_url + 'publicnewslist/attach/' + work_info_id_pri;
                }
                //ajax_pagination();
            }
        })
