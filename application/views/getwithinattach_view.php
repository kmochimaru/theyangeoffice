            ...
            <div class="card-body">
                ...
                <div class=" col-md-12 text-right">
                    <span style="display: none;" id="btn_send">
                        <a href="<?php echo base_url() . 'getwithinwaiting/send/' . $data->work_info_id_pri; ?>" class="btn btn-warning">
                            <i class="fa fa-send"></i> ส่งเอกสาร
                        </a>
                        <a href="<?php echo base_url() . 'getwithinwaiting/sendsort/' . $data->work_info_id_pri; ?>" class="btn btn-outline-warning">
                            <i class="fa fa-send"></i> ส่งตามลำดับเส้นทาง
                        </a>
                    </span>
                    <a href="<?php echo base_url() . 'getwithinwaiting/edit/' . $data->work_info_id_pri; ?>" class="btn btn-info" class="dropdown-item small"><i class="fa fa-edit"></i> แก้ไขเอกสาร</a>
                    <!-- <span style="float: right; margin-right: 5px;"> -->
                    <a href="<?php echo base_url() . 'getwithinwaiting/notupload_attach/' . $data->work_info_id_pri; ?>" class="btn btn-danger">
...
<script>
    ...
    function upload_attach() {
        ...
        url = service_base_url + 'getwithinwaiting/upload_attach/' + $('#work_info_id_pri').val();
        $.ajax({
            url: url,
            type: 'POST',
            contentType: false,
    ...
    function ajax_page() {
        ...
        $('#result_page').html('<div style="text-align:center; padding:80px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');
        $.ajax({
            url: service_base_url + 'getwithinwaiting/ajax_page',
            type: 'post',
            data: {
    ...
    function delete_file(id, name) {
        url = service_base_url + 'getwithinwaiting/delete_file';
        $.ajax({
            url: url,
            method: 'POST',
            data: {
    ...
    //     $.ajax({
    //         url: service_base_url + 'getwithinwaiting/modal_regisnumber',
    //         type: 'POST',
    //         data: {
    ...
    //     $.ajax({
    //         url: service_base_url + 'getwithinwaiting/regisnumber',
    //         type: 'POST',
    //         data: {
    //             work_info_id_pri: work_info_id_pri,
    //         },
    ...
    //                 window.location.href = service_base_url + 'getwithinwaiting';
    //             } else {
    //                 notification('error', 'ไม่สำเร็จ', 'ลงทะเบียนเอกสารไม่สำเร็จ!');
    //                 window.location.href = service_base_url + 'getwithinwaiting/attach/' + work_info_id_pri;
    //             }
    //             //ajax_pagination();
    //         }
    //     })
