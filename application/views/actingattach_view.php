            ...
            <div class="card-body">
                <h4 class="card-title">
                    ...
                    <span style="float: right; margin-right: 5px;">
                        <a href="<?php echo base_url() . 'actingwaiting/notupload_attach/' . $data->work_info_id_pri; ?>" class="btn btn-sm btn-rounded btn-danger">
                ...
                <div class=" col-md-12 text-right">
                    <a href="<?php echo base_url() . 'actingwaiting/edit/' . $data->work_info_id_pri; ?>" class="btn btn-info" class="dropdown-item small"><i class="fa fa-edit"></i> แก้ไขเอกสาร</a>                  
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12 text-right">
                        <a href="<?php echo base_url() . 'actingwaiting'; ?>" class="btn btn-secondary" ><i class="fa fa-arrow-left"></i>&nbsp;กลับ</a>                        
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function upload_attach() {
        ...
        url = service_base_url + 'actingwaiting/upload_attach/' + $('#work_info_id_pri').val();
        $.ajax({
            url: url,
            type: 'POST',
            contentType: false,
    ...
    function ajax_page() {
        ...
        $('#result_page').html('<div style="text-align:center; padding:80px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');
        $.ajax({
            url: service_base_url + 'actingwaiting/ajax_page',
            type: 'post',
            data: {
    ...
    function delete_file(id, name) {
        url = service_base_url + 'actingwaiting/delete_file';
        $.ajax({
            url: url,
            method: 'POST',
            data: {
    ...
    function modal_regisnumber(work_info_id_pri) {
        $.ajax({
            url: service_base_url + 'actingwaiting/modal_regisnumber',
            type: 'POST',
            data: {
    ...
    function regisnumber(work_info_id_pri) {
        ...
        $.ajax({
            url: service_base_url + 'actingwaiting/regisnumber',
            type: 'POST',
            data: {
                work_info_id_pri: work_info_id_pri,
            },
            success: function (response) {
                ...
                if (response == 1) {
                    ...
                    window.location.href = service_base_url + 'actingwaiting';
                } else {
                    notification('error', 'ไม่สำเร็จ', 'ลงทะเบียนเอกสารไม่สำเร็จ!');
                    window.location.href = service_base_url + 'actingwaiting/attach/' + work_info_id_pri;
                }
            }
        })
    }
