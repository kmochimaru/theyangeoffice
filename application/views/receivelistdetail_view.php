      ...
      <div class="card-body">
        <h4 class="card-title">
          ...
          <span style="float: right">
            <a href="<?php echo base_url() . 'receivelist'; ?>" class="btn btn-sm btn-outline-secondary"><i class="fa fa-arrow-left"></i> กลับ</a>
          </span>
        </h4>
        <hr>
        <div class="row">
          <div class="col-md-12 text-left">
            <div class="form-group row">
              ...
              <div class="col-sm-10 m-t-0">
                <?php
                $files = $this->receivelist_model->get_workinfofile($data->work_info_id_pri);
                ?>
        ...
        <?php if ($this->receivelist_model->checkworkprocess($work_process_id_pri)->row()->work_process_receive == 0) { ?>
          <hr>
          <div class="row">
          ...
          <div class="row">
            <div class="col-md-12 text-right">
              <?php if ($data->state_info_id != 6) { ?>
                <?php
                if ($workprocess->work_process_status_id == 1) {
                  if ($workprocess->work_process_receive_user_id != null) {
                    if ($workprocess->work_process_receive_user_id == $this->session->userdata('user_id') || $workprocess->special_command_id == 1) {
                ?>
                ...
                <a href="<?php echo base_url() . 'receivelist/attach/' . $work_process_id_pri; ?>" class="btn btn-outline-primary"><i class="fa fa-paperclip"></i>&nbsp;เพิ่มไฟล์เอกสาร</a>
                <?php
                if ($workprocess->work_process_sendtype == 1) {
                  if ($workprocess->work_process_status_id == 1) {
                    if ($data->work_type_id != 5) {
                ?>
                      <!-- <a href="<?php //echo base_url() . 'receivelist/senduser/' . $data->work_info_id_pri . '/' . $work_process_id_pri; ?>" class="btn btn-outline-info"><i class="fa fa-send"></i>&nbsp;ส่งถึงผู้ปฏิบัติงาน</a> -->
                    <?php } ?>
                    <a href="<?php echo base_url() . 'receivelist/send/' . $data->work_info_id_pri . '/' . $work_process_id_pri; ?>" class="btn btn-info"><i class="fa fa-send"></i>&nbsp;แจกจ่ายส่งต่อ</a>
                    <?php
                  } else {
                    if ($workprocess->work_process_sendstatus == 0) {
                      if ($this->receivelist_model->checkwithinprocesslast($data->work_info_id_pri)->work_process_sort != $workprocess->work_process_sort) {
                    ?>
                        <button onclick="modal_sendstatus();" class="btn btn-info"><i class="fa fa-telegram"></i>&nbsp;ส่งต่อตามเส้นทาง</button>
                        <a href="<?php echo base_url() . 'receivelist/changesend/' . $data->work_info_id_pri . '/' . $work_process_id_pri; ?>" class="btn btn-outline-info"><i class="fa fa-send-o"></i>&nbsp;เปลี่ยนเส้นทาง</a>
                        <?php
                      } else {
                        if ($data->work_type_id != 5) {
                        ?>
                ...
                if ($data->work_type_id != 5) {
                  // ไม่ให้ตีกลับซ้ำ
                  if ($this->receivelist_model->checkwithinbounce($data->work_info_id_pri)->num_rows() == 0) {
                    if ($workprocess->work_process_sendtype == 1) {
                      ?>
...
<?php
$work_process_receive = $this->receivelist_model->checkworkprocess($work_process_id_pri)->row()->work_process_receive;
if ($work_process_receive == 1) {
?>
  <div class="row">
    <div class="col-12">
      <div class="card">
                  ...
                  <tbody>
                    ...
                    ?>
                        <tr>
                          ...
                          <td><a href="<?php echo base_url() . 'store/file/' . $file->work_info_file_id; ?>" target="_blank"><?php echo $file->work_info_file_oldname; ?></a></td>
                          <td colspan="2">ต้นเรื่อง</td>
                          <td><?php echo $this->receivelist_model->get_user($file->user_id)->user_fullname; ?></td>
                    ...
                    if ($this->receivelist_model->checkworkprocess($work_process_id_pri)->row()->work_process_receive == 1) {
                      $workprocessfiles = $this->receivelist_model->get_workprocessfofile($data->work_info_id_pri);
                      if ($workprocessfiles->num_rows() > 0) {
                        if ($this->receivelist_model->check_workprocessfofileinfo($data->work_info_id_pri) > 0 || $this->receivelist_model->check_workprocessfofileprocess($work_process_id_pri) > 0 || $this->receivelist_model->check_workprocessfofiledepoff($data->work_info_id_pri) > 0) {
                        ...
                        ?>
                                <tr>
                                  ...
                                  <td><a style="padding-right: 5px" href="<?php echo base_url() . 'store/workprocessfile/' . $workprocessfile->work_process_file_id; ?>" title="Item Title" class="<?php echo ($workprocessfile->file_type_check != 2) ? ($workprocessfile->file_type_check != 1) ? 'fancybox' : 'fancyboxpdf' : ''; ?>" target="_blank"><?php echo $workprocessfile->work_process_file_oldname; ?></a></td>
                                  <?php
                                  if ($workprocessfile->work_process_file_send == 1) {
                                    echo '<td colspan="2">ต้นเรื่อง (เอกสารเพิ่มเติม)</td>';
                                  } else {
                                    $dep_off = $this->receivelist_model->getdep_off_id($workprocessfile->dep_off_id);
                                    echo '<td>' . $dep_off->dep_name . '</td><td>' . $dep_off->officer_name_display . '</td>';
                                  }
                                  ?>
                                  <td><?php echo $this->receivelist_model->get_user($workprocessfile->user_id)->user_fullname; ?></td>
                              ...
                              ?>
                                <tr>
                                  ...
                                  <td><?php echo $workprocessfile->work_process_file_oldname; ?></td>
                                  <?php
                                  if ($workprocessfile->work_process_file_send == 1) {
                                    echo '<td colspan="2">ต้นเรื่อง (เอกสารเพิ่มเติม)</td>';
                                  } else {
                    ...
                    $workuserfofiles = $this->receivelist_model->get_workuserfofile($data->work_info_id_pri);
                      ...
                      foreach ($workuserfofiles->result() as $workuserfofile) {
                        ...
                        <tr>
                          ...
                          <td><a style="padding-right: 5px" href="<?php echo base_url() . 'store/workuserfile/' . $workuserfofile->work_user_file_id; ?>" title="Item Title" class="<?php echo ($workuserfofile->file_type_check != 2) ? ($workuserfofile->file_type_check != 1) ? 'fancybox' : 'fancyboxpdf' : ''; ?>" target="_blank">
                              <?php echo $workuserfofile->work_user_file_oldname; ?></a></td>
                          <td>
                            <?php
                            $file_work_process = $this->receivelist_model->get_workuserid($workuserfofile->work_user_id)->row();
                            $dep_off = $this->receivelist_model->getdep_off_id($file_work_process->dep_off_id);
                            echo $dep_off->dep_name;
                            ?>
                          </td>
                          <td><?php echo 'ผู้ปฏิบัติงาน'; ?></td>
                          <td><?php echo $this->receivelist_model->get_user($workuserfofile->user_id)->user_fullname; ?></td>
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  <?php
                  $i = 1;
                  $rows = $this->receivelist_model->get_withinprocess($data->work_info_id_pri);
                  if ($rows->num_rows() > 0) {
                  ...
                  ?>
                      <tr>
                        ...
                        <td style="font-weight: bold;">
                          ...
                          <?php } ?>
                          <?php
                          $checks = $this->receivelist_model->checkwithinprocess($row->work_info_id_pri, ($row->work_process_sort - 1), 0);
                        ...
                        <td style="border-right: 1px solid whitesmoke;">
                          ...
                          if ($row->work_process_id_to == null) {
                            if ($row->work_user_id != null) {
                              echo $this->receivelist_model->get_user($row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )';
                            } else {
                              echo $this->receivelist_model->getdep_off_id($data->dep_off_id)->dep_name . '<br> ( ' . $this->receivelist_model->getdep_off_id($data->dep_off_id)->officer_name_display . ' )';
                            }
                          } else {
                            $dep_off_to = $this->receivelist_model->getdep_off_id($this->receivelist_model->getworkprocess($row->work_process_id_to)->row()->dep_off_id);
                        ...
                        <td>
                          <?php
                          $dep_off_to = $this->receivelist_model->getdep_off_id($row->dep_off_id);
                      ...
                      <tr>
                        ...
                        <?php $special_command_name = $this->receivelist_model->ref_special_command($row->special_command_id)->row()->special_command_name; ?>
                      ...
                      <?php
                      $i++;
                      $user_rows = $this->receivelist_model->get_workuser($row->work_process_id_pri);
                      ...
                      ?>
                          <tr>
                            ...
                            <td style="border-right: 1px solid whitesmoke;">
                              <?php
                              $dep_off_id = $this->receivelist_model->getdep_off_id($this->receivelist_model->getworkprocess($row->work_process_id_pri)->row()->dep_off_id);
                              echo $dep_off_id->dep_name . '<br> ( ' . $dep_off_id->officer_name_display . ' )';
                              ?>
                            </td>
                            <td><?php echo $this->receivelist_model->get_user($user_row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )'; ?></td>
                            <td class="text-center"><?php echo '-'; ?></td>
                            <?php if ($user_row->work_user_status_id != 1) { ?>
                              <td><?php echo $this->receivelist_model->get_user($user_row->user_id)->user_fullname; ?></td>
...
<div id="modal_comment" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form id="form_modal_comment" method="post" action="<?php echo base_url('receivelist/add_comment'); ?>" enctype="multipart/form-data" autocomplete="off">
        <div class="modal-header">
          <h4 class="modal-title"><i class="fa fa-pencil-square-o"></i> บันทึกงาน</h4>
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        </div>
        <?php
        $row_comment = $this->receivelist_model->getworkprocess($work_process_id_pri)->row();
        ?>
        <div class="modal-body">
          <div class="row">
            <div class="form-group col-md-4 col-6">
              <label class="control-label">สถานะตอบรับ <span class="text-danger">*</span></label>
              <select name="special_command_id" class="form-control form-control-sm" required="">
                <option value=""><?php echo 'รอตอบรับ'; ?></option>
                <?php
                foreach ($this->receivelist_model->ref_special_command()->result() as $special_command) {
                  if ($special_command->special_command_id != 1) {
                ?>
                    <option value="<?php echo $special_command->special_command_id; ?>" <?php echo $row_comment->special_command_id == $special_command->special_command_id ? 'selected' : '' ?>><?php echo $special_command->special_command_name; ?></option>
                <?php
                  }
                }
                ?>
            ...
            <div class="form-group col-md-8 col-6">
              <label class="control-label">เลือกคำบันทึกงาน</label>
              <select id="word_comment" class="form-control form-control-sm" onchange="change_word_comment()">
                <option value="">ไม่ระบุ</option>
                <?php
                foreach ($this->receivelist_model->word_comment()->result() as $wc) {
                ?>
                  <option value="<?php echo $wc->word_comment; ?>"><?php echo $wc->word_comment; ?></option>
                <?php
                }
                ?>
              </select>
          ...
          <div class="form-group">
            ...
            <textarea name="work_process_receive_comment" id="work_process_receive_comment" class="form-control form-control-sm" rows="4"><?php echo $row_comment->work_process_receive_comment ?></textarea>
          </div>
          <?php
          $user_signature_path = $this->receivelist_model->get_user($this->session->userdata('user_id'))->user_signature_path;
          $work_process_receive_signature = $row_comment->work_process_receive_signature;
          ...
          <div class="row">
            ...
            <div class="form-group col-md-7">
              <input name="work_process_act_for_flag" id="work_process_act_for_flag" type="checkbox" value="<?php echo $row_comment->work_process_act_for_flag; ?>" <?php echo ($row_comment->work_process_act_for_flag == '1' ? 'checked' : ''); ?> class="filled-in chk-col-blue" onchange="act_for_check();">
              <label for="work_process_act_for_flag">&nbsp; เลือกรักษาการแทน</label>
              <br>
              <div id="act_for_flag_word_show" style="display: none;">
                <input name="act_for_flag_word" id="act_for_flag_word1" type="radio" value="รักษาราชการแทน" onchange="act_for_word('รักษาราชการแทน');" class="filled-in chk-col-green">
                <label for="act_for_flag_word1">&nbsp; รักษาราชการแทน</label>
                <input name="act_for_flag_word" id="act_for_flag_word2" type="radio" value="ปฎิบัติราชการแทน" onchange="act_for_word('ปฎิบัติราชการแทน');" class="filled-in chk-col-green">
                <label for="act_for_flag_word2">&nbsp; ปฎิบัติราชการแทน</label>
                <br>
              </div>
              <label class="control-label">ตำแหน่งรักษาการแทน</label>
              <input name="work_process_act_for_position" id="work_process_act_for_position" type="text" value="<?php echo ($row_comment->work_process_act_for_flag == '1' ? $row_comment->work_process_act_for_position : ''); ?>" class="form-control form-control-sm" <?php echo ($row_comment->work_process_act_for_flag == '0' ? 'readonly=""' : ''); ?> autocomplete="new-text">
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <div class="row">
              <div class="col-lg-6">
                ...
                <input type="password" name="pin_key" id="pin_key" pattern="[0-9]{6}" maxlength="6" class="form-control form-control-sm" data-parsley-error-message="กรุณากรอกรหัส PIN ตัวเลข 6 หลัก" required="" autocomplete="new-pin">
              </div>
            </div>
            <!-- <div class="row">
                            <div class="col-lg-12">
...
<div id="modal_back" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form id="form_modal_back" method="post" action="<?php echo base_url('receivelist/add_back'); ?>" enctype="multipart/form-data" autocomplete="off">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input type="hidden" name="work_process_id_pri" value="<?php echo $work_process_id_pri; ?>">
            <textarea name="work_process_receive_comment_1" id="work_process_receive_comment_1" class="form-control form-control-sm" rows="4" required=""><?php echo $this->receivelist_model->getworkprocess($work_process_id_pri)->row()->work_process_receive_commentback ?></textarea>
          </div>
        </div>
        <div class="modal-footer">
...
<script>
  ...
  function modal_receive() {
    $.ajax({
      url: service_base_url + 'receivelist/modal_receive',
      type: 'POST',
      data: {
  ...
  function modal_sendstatus() {
    $.ajax({
      url: service_base_url + 'receivelist/modal_sendstatus',
      type: 'POST',
      data: {
        work_process_id_pri: $('#work_process_id_pri').val(),
      },
  ...
  function receivelist(work_process_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'receivelist/receive',
      type: 'POST',
      data: {
      ...
      success: function(response) {
        ...
        if (response == 0) {
          ...
          window.location.href = '<?php echo base_url() . 'receivelist/detail/'; ?>' + work_process_id_pri;
        }
      }
    })
  }
  ...
  function receivesend(work_process_id_pri) {
    ...
    $('#btn-modal-submit-loading').show();
    $.ajax({
      url: service_base_url + 'receivelist/receivesend',
      type: 'POST',
      data: {
        work_process_id_pri: work_process_id_pri
      },
  ...
  function act_for_check() {
    if ($('#work_process_act_for_flag').is(':checked') == false) {
      $('#work_process_act_for_flag').val(0);
      $('#work_process_act_for_position').val('');
      $("#work_process_act_for_position").prop("readonly", true);
      $("#work_process_act_for_position").prop("required", false);
      $('#act_for_flag_word_show').css('display', 'none');
    } else {
      $('#work_process_act_for_flag').val(1);
      $("#work_process_act_for_position").prop("readonly", false);
      $("#work_process_act_for_position").prop("required", true);
      $('#act_for_flag_word_show').css('display', 'block');
    }
  }

  function act_for_word(word) {
    $('#work_process_act_for_position').val(word);
  }

  function change_word_comment() {
    $('#work_process_receive_comment').val($('#word_comment').val());
  }
</script>