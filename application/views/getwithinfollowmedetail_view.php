      ...
      <div class="card-body">
        <h4 class="card-title">
          ...
          <span style="float: right">
            <a href="<?php echo base_url() . 'getwithinfollowme'; ?>" class="btn btn-sm btn-outline-secondary"><i class="fa fa-arrow-left"></i> กลับ</a>
          </span>
        </h4>
        <hr>
        <div class="row">
          <div class="col-md-12 col-sm-12 text-left">
            <div class="row">
              ...
              <div class="col-sm-10 m-t-0">
                <?php
                $files = $this->getwithinfollowme_model->get_workinfofile($data->work_info_id_pri);
                ?>
        ...
        <div class="row">
          <div class="col-md-12 text-right">
            <?php if ($data->state_info_id <= 3) { ?>
              <a href="<?php echo base_url() . 'printout/docketinfo/' . $data->work_info_code; ?>" class="btn btn-success fancyboxpdf" target="_blank"><i class="fa fa-print"></i>&nbsp;พิมพ์เอกสาร</a>
            <?php } else { ?>
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  ...
                  if ($files->num_rows() > 0) {
                    foreach ($files->result() as $file) { ?>
                      <tr>
                        ...
                        <td><?php echo $this->getwithinfollowme_model->get_user($file->user_id)->user_fullname; ?></td>
                  ...
                  $workprocessfiles = $this->getwithinfollowme_model->get_workprocessfofile($data->work_info_id_pri);
                    ...
                    ?>
                      <tr>
                        ...
                        if ($workprocessfile->work_process_file_send == 1) {
                          ...
                          $dep_off = $this->getwithinfollowme_model->getdep_off_id($workprocessfile->dep_off_id);
                          echo '<td>' . $dep_off->dep_name . '</td><td>' . $dep_off->officer_name_display . '</td>';
                        }
                        ?>
                        <td><?php echo $this->getwithinfollowme_model->get_user($workprocessfile->user_id)->user_fullname; ?></td>
                  ...
                  $workuserfofiles = $this->getwithinfollowme_model->get_workuserfofile($data->work_info_id_pri);
                  if ($workuserfofiles->num_rows() > 0) {
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <?php
                          $file_work_process = $this->getwithinfollowme_model->get_workuserid($workuserfofile->work_user_id)->row();
                          $dep_off = $this->getwithinfollowme_model->getdep_off_id($file_work_process->dep_off_id);
                          echo $dep_off->dep_name;
                          ?>
                        </td>
                        <td><?php echo 'ผู้ปฏิบัติงาน'; ?></td>
                        <td><?php echo $this->getwithinfollowme_model->get_user($workuserfofile->user_id)->user_fullname; ?></td>
          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tbody>
                <?php
                $i = 1;
                $rows = $this->getwithinfollowme_model->get_withinprocess($work_info_id_pri);
                if ($rows->num_rows() > 0) {
                ...
                ?>
                    <tr>
                      ...
                      <td style="font-weight: bold;">
                        ...
                        <?php } ?>
                        <?php
                        $checks = $this->getwithinfollowme_model->checkwithinprocess($row->work_info_id_pri, ($row->work_process_sort - 1), 0);
                      ...
                      <td style="border-right: 1px solid whitesmoke;">
                        ...
                        if ($row->work_process_id_to == null) {
                          if ($row->work_user_id != null) {
                            echo  $this->getwithinfollowme_model->get_user($row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )';
                          } else {
                            echo $this->getwithinfollowme_model->getdep_off_id($data->dep_off_id)->dep_name . '<br> ( ' . $this->getwithinfollowme_model->getdep_off_id($data->dep_off_id)->officer_name_display . ' )';
                          }
                        } else {
                          $dep_off_to = $this->getwithinfollowme_model->getdep_off_id($this->getwithinfollowme_model->getworkprocess($row->work_process_id_to)->row()->dep_off_id);
                      ...
                      <td>
                        <?php
                        $dep_off_to = $this->getwithinfollowme_model->getdep_off_id($row->dep_off_id);
                    ...
                    <tr>
                      ...
                      <?php $special_command_name = $this->getwithinfollowme_model->ref_special_command($row->special_command_id)->row()->special_command_name; ?>
                    ...
                    <?php
                    $i++;
                    $user_rows = $this->getwithinfollowme_model->get_workuser($row->work_process_id_pri);
                    ...
                    ?>
                        <tr>
                          ...
                          <td style="border-right: 1px solid whitesmoke;">
                            <?php
                            $dep_off_id = $this->getwithinfollowme_model->getdep_off_id($this->getwithinfollowme_model->getworkprocess($row->work_process_id_pri)->row()->dep_off_id);
                            echo $dep_off_id->dep_name . '<br> ( ' . $dep_off_id->officer_name_display . ' )';
                            ?>
                          </td>
                          <td><?php echo  $this->getwithinfollowme_model->get_user($user_row->user_id)->user_fullname . '<br> ( ผู้ปฏิบัติงาน )'; ?></td>
                          <td class="text-center"><?php echo '-'; ?></td>
                          <?php if ($user_row->work_user_status_id != 1) { ?>
                            <td><?php echo $this->getwithinfollowme_model->get_user($user_row->user_id)->user_fullname; ?></td>
...
<script>
  ...
  function unfollow(work_info_id_pri) {
    ...
    $.ajax({
      url: service_base_url + 'getwithinfollowme/unfollow',
      type: 'POST',
      data: {
      ...
      success: function(response) {
        if (response == 1) {
          ...
          setTimeout(function() {
            window.location.href = '<?php echo base_url() . 'getwithinfollowme'; ?>';
          }, 2500);
        } else {
          $('#modal_unfollow').modal('hide');
