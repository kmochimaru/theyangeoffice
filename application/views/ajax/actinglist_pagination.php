    ...
    <tbody>
      ...
      ?>
          <tr>
            ...
            <td class="text-center">
              <a href="<?php echo base_url() . 'actinglist/detail/' . $row->work_info_code; ?>" title="รายละเอียด" data-toggle="tooltip">
            ...
            <td>
              <?php
              $files = $this->actinglist_model->get_workinfofile($row->work_info_id_pri);
              if ($files->num_rows() > 0) {
            ...
            <td class="text-center" style="font-weight: bold;">
              ...
              <?php } else if ($row->state_info_id == 6) { ?>
                <span class="badge badge-success"><i class="fa fa-bullhorn"></i> <?php echo 'ประกาศแล้ว'; ?></span>
            ...
            <td class="text-center">
              <div class="btn-group">
                ...
                <div class="dropdown-menu">
                  <a href="<?php echo base_url() . 'actinglist/detail/' . $row->work_info_code; ?>" title="รายละเอียด" class="dropdown-item small"><i class="fa fa-navicon"></i> รายละเอียด</a>
                  <?php if ($row->state_info_id <= 4) { ?>
                    <a href="<?php echo base_url() . 'actinglist/attach/' . $row->work_info_code; ?>" class="dropdown-item small"><i class="mdi mdi-paperclip"></i> เพิ่มไฟล์แนบ</a>
                  <?php } ?>
                  <a href="<?php echo base_url() . 'printout/docketprocess/' . $row->work_info_code; ?>" class="dropdown-item small fancyboxpdf" target="_blank"><i class="fa fa-print"></i> พิมพ์เอกสาร</a>
                  <?php
                  if ($row->state_info_id == 4) {
                    if ($this->actinglist_model->checkchangestatus($row->work_info_id_pri) == 0) {
                  ?>
                  ...
                  <?php } ?>
                  <?php if ($row->state_info_id == 5) { ?>
                    <a href="javascript:void(0)" onclick="modal_publicstatus('<?php echo $row->work_info_id_pri; ?>');" class="dropdown-item small"><i class="fa fa fa-bullhorn"></i> ประกาศรักษาการ</a>
                  <?php } ?>
                </div>
              </div>
            </td>
