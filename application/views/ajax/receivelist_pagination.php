        ...
        <tbody>
            ...
            if ($data->num_rows() > 0) {
                ...
                foreach ($data->result() as $rows) {
                    $row = $this->receivelist_model->getData($rows->work_process_id_pri)->row();
            ...
            ?>
                        <tr>
                            ...
                            <td style="font-weight: bold;">
                                ...
                                <?php } else if ($row->work_process_receive == 2) { ?>
                                    <i class="fa fa-envelope text-warning"></i>
                                    <span class="text-danger"><?php echo $this->receivelist_model->check_info_depoff($row->work_info_id_pri, $row->dep_off_id)->num_rows() > 1 ? $this->receivelist_model->check_info_depoff($row->work_info_id_pri, $row->dep_off_id)->num_rows() : ''; ?></span>
                                <?php } else { ?>
                                    <i class="fa fa-envelope text-warning"></i>
                                <?php } ?>
                                ...
                                <?php } ?>
                                <?php if ($row->special_command_id == 1) {
                                    if ($row->work_process_receive == 1) {
                                        if ($row->state_info_id == 4 || $row->state_info_id == 5) { ?>
                                            <i class="fa fa-pencil text-primary"></i>
                                            <?php if ($this->receivelist_model->check_workprocessto($rows->work_process_id_pri) == 0) { ?>
                            ...
                            <td>
                                <?php echo $this->receivelist_model->getdep_off($row->work_info_dep_id_pri)->row()->dep_name; ?>
                                <br>
                                <span class="small"><?php echo $this->receivelist_model->getdep_off_id($row->work_info_dep_off_id)->officer_name; ?></span>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <a href="<?php echo base_url() . 'receivelist/detail/' . $row->work_process_id_pri . '/1'; ?>" class="btn btn-sm btn-info"><i class="fa fa-envelope-open"></i> เปิด</a>
                            </td>
                        </tr>
                        <?php
                        $i++;
