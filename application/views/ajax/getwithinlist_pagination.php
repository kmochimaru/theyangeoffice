    ...
    <tbody>
      ...
      ?>
          <tr>
            ...
            <td class="text-center">
              <a href="<?php echo base_url() . 'getwithinlist/detail/' . $row->work_info_code; ?>" title="รายละเอียด" data-toggle="tooltip">
            ...
            <td>
              <?php
              $files = $this->getwithinlist_model->get_workinfofile($row->work_info_id_pri);
              if ($files->num_rows() > 0) {
            ...
            <td class="text-center">
              <div class="btn-group">
                ...
                <div class="dropdown-menu">
                  <a href="<?php echo base_url() . 'getwithinlist/detail/' . $row->work_info_code; ?>" title="รายละเอียด" class="dropdown-item small"><i class="fa fa-navicon"></i> รายละเอียด</a>
                  <?php if ($row->state_info_id <= 4) { ?>
                    <a href="<?php echo base_url() . 'getwithinlist/attach/' . $row->work_info_code; ?>" class="dropdown-item small"><i class="mdi mdi-paperclip"></i> เพิ่มไฟล์แนบ</a>
                  <?php } ?>
                  ...
                  if ($row->state_info_id == 4) {
                    if ($this->getwithinlist_model->checkchangestatus($row->work_info_id_pri) == 0) {
                  ?>
                    ...
                    <?php } ?>
                  <?php } ?>
                  <?php if ($row->state_info_id == 4 || $row->state_info_id == 5) { ?>
                    <a href="javascript:void(0)" onclick="modal_closestatus('<?php echo $row->work_info_id_pri; ?>');" class="dropdown-item small"><i class="fa fa-power-off"></i> ปิดงาน</a>
                  <?php } ?>
                </div>
              </div>
            </td>
