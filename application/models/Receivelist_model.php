...
/**
 * Description of Receivelist_model
 * @author nut
 */
class Receivelist_model extends CI_Model {
    ...
    public function count_pagination($filter) {
        ...
        if ($filter['status_id'] != '') {
            ...
            } else if ($filter['status_id'] == 99) {
                $this->db->where_in('work_process.special_command_id', 1);
                $this->db->where_in('work_info.state_info_id', array(4, 5, 6, 8));
            }
        } else {
            $this->db->where_in('work_info.state_info_id', array(4, 5, 6, 8));
    ...
    public function check_info_depoff_id($work_info_id_pri, $dep_off_id) {
        $this->db->select('*');
        $this->db->from('work_process');
    ...
    public function check_info_depoff($work_info_id_pri, $dep_off_id) {
        $this->db->select('*');
        $this->db->from('work_process');
        $this->db->where('work_process.work_info_id_pri', $work_info_id_pri);
        $this->db->where('work_process.dep_off_id', $dep_off_id);
    ...
    public function word_comment($word_id = null) {
        $this->db->select('*');
        $this->db->from('word_comment');
        if ($word_id != null) {
            $this->db->where('word_comment.word_id', $word_id);
            $this->db->limit(1);
        }
        $this->db->order_by('word_comment.word_sort');
        return $this->db->get();
    }
    ...
    public function getworkprocessdep($work_info_id_pri) {
        $this->db->select('work_process.work_process_id_pri,work_process.work_process_id_pri');
        $this->db->from('work_process');
        $this->db->join('dep_off', 'work_process.dep_off_id = dep_off.dep_off_id');
        $this->db->join('department', 'dep_off.dep_id_pri = department.dep_id_pri');
    ...
    public function checkwithinbounce($work_info_id_pri) {
        $this->db->select('*');
        $this->db->from('work_process');
        $this->db->join('work_info', 'work_process.work_info_id_pri = work_info.work_info_id_pri');
        $this->db->where('work_process.work_info_id_pri', $work_info_id_pri);
    ...
    public function check_workprocessto($work_process_id_pri) {
        $this->db->select('*');
        $this->db->from('work_process');
        $this->db->where('work_process.work_process_id_to', $work_process_id_pri);
        return $this->db->get()->num_rows();
    }
}
