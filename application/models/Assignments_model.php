...
class Assignments_model extends CI_Model {
    ...
    public function count_pagination($filter) {
        ...
        $this->db->where('work_user.work_user_giver_id', $this->session->userdata('user_id'));
    ...
    public function getworkprocessworkuserid($work_user_id) {
        $this->db->select('work_process.work_process_id_pri');
        $this->db->from('work_process');
        $this->db->where('work_process.work_user_id', $work_user_id);
        return $this->db->get();
    }
