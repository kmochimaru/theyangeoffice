...
/**
 * Description of Servieclist_model
 *
 * @author nut
 */
class Servieclist_model extends CI_Model {
    ...
    public function count_pagination($filter) {
        ...
        $this->db->join('help_desk_team', 'help_desk_team.service_info_id_pri = service_info.service_info_id_pri');
        $this->db->join('user', 'service_info.user_id = user.user_id');
        $this->db->where('help_desk_team.user_id', $this->session->userdata('user_id'));
        ...
        $this->db->group_by('service_info.service_info_id_pri');
    ...
    public function get_service_user($service_info_id_pri) {
        $this->db->select('*');
        $this->db->from('service_user');
        //$this->db->join('user', 'service_user.user_id = user.user_id');
        if ($service_info_id_pri != null) {
            $this->db->where('service_user.service_info_id_pri', $service_info_id_pri);
        }
        return $this->db->get();
    }
    ...
    public function get_help_desk_active($service_id, $user_id) {
