...
/**
 * Description of Getwithin_model
 *
 * @author nut
 */
class Getwithin_model extends CI_Model {
    ...
    public function ref_work_type($work_type_id = null) {
        ...
        if ($work_type_id != null) {
            ...
            $this->db->where_in('ref_work_type.work_type_id', array(1, 6));
        }
    ...
    public function getAgency($agency_id_pri = null) {
        $this->db->select('*');
        $this->db->from('agency');
        if ($agency_id_pri != null) {
            $this->db->where('agency.agency_id_pri', $agency_id_pri);
        }
        $this->db->where('agency.dep_status_id', 1);
        return $this->db->get();
    }
    ...
    public function check_info_number($filter) {
        ...
        if ($filter['work_info_no'] != '') {
            $this->db->where(" (
                work_info.work_info_no LIKE '%" . $filter['work_info_no'] . "%'
            ) ");
        }
