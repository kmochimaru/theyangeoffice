...
//		if ($currblk['inner_width'] < $mw) { throw new MpdfException("DIV is too narrow for text to fit!"); }
			...
			case 'IMG':
				$this->mpdf->ignorefollowingspaces = false;
				if ($this->mpdf->processBar) {
					$this->mpdf->UpdateprocessBar(1, '', 'IMG');
				} // *process-BAR*
				$objattr = array();
				$objattr['margin_top'] = 0;
				$objattr['margin_bottom'] = 0;
	...
	{ // mPDF 6
		...
		if ($tag == 'FONT' || $tag == 'SPAN' || $tag == 'CODE' || $tag == 'KBD' || $tag == 'SAMP' || $tag == 'TT' || $tag == 'VAR' || $tag == 'INS' || $tag == 'STRONG' || $tag == 'CITE' || $tag == 'SUB' || $tag == 'SUP' || $tag == 'S' || $tag == 'STRIKE' || $tag == 'DEL' || $tag == 'Q' || $tag == 'EM' || $tag == 'B' || $tag == 'I' || $tag == 'U' | $tag == 'SMALL' || $tag == 'BIG' || $tag == 'ACRONYM' || $tag == 'MARK' || $tag == 'TIME' || $tag == 'process' || $tag == 'METER' || $tag == 'BDO' || $tag == 'BDI'
		) {

			$annot = false; // mPDF 6
			$bdf = false; // mPDF 6
			// mPDF 5.7.3 Inline tags
			if ($tag == 'process' || $tag == 'METER') {
		...
		if ($tag == 'TABLE') { // TABLE-END (
			if ($this->mpdf->processBar) {
				$this->mpdf->UpdateprocessBar(1, '', 'TABLE');
			} // *process-BAR*
			if ($this->mpdf->processBar) {
				$this->mpdf->UpdateprocessBar(7, 0, '');
			} // *process-BAR*
			$this->mpdf->lastoptionaltag = '';
			unset($this->mpdf->cssmgr->tablecascadeCSS[$this->mpdf->cssmgr->tbCSSlvl]);
			$this->mpdf->cssmgr->tbCSSlvl--;
			...
			if ($this->mpdf->processBar) {
				$this->mpdf->UpdateprocessBar(7, 10, ' ');
			} // *process-BAR*
			...
			if ($this->mpdf->processBar) {
				$this->mpdf->UpdateprocessBar(7, 20, ' ');
			} // *process-BAR*
			if ($this->mpdf->table[1][1]['overflow'] == 'visible') {
				...
				if ($this->mpdf->processBar) {
					$this->mpdf->UpdateprocessBar(7, 30, ' ');
				} // *process-BAR*
			...
			if ($this->mpdf->processBar) {
				$this->mpdf->UpdateprocessBar(7, 100, ' ');
			} // *process-BAR*
