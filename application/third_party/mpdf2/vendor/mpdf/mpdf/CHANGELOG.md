...
- CSS not applying cascaded styles on `<A>` elements - [changed MergeCSS() type to INLINE for 'A', LEGEND, METER and process]
...
- as in-line elements: `<mark><time><meter><process>`
- `<mark>` has a default CSS set in config.php to yellow highlight
- `<meter>` and `<process>` support attributes as for HTML5
- custom appearances for `<meter>` and `<process>` can be made by editing `classes/meter.php` file
- `<meter>` and `<process>` suppress text inside the tags
...
- SVG setting font-size as percent on successive `<text>` elements gives processively smaller text
