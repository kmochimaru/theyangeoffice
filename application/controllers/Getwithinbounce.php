...
/**
 * Description of Getwithinbounce
 * @author nut
 */
class Getwithinbounce extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('getwithinbounce_model');
    ...
    public function index() {
        ...
        $this->renderView('getwithinbounce_view', $data);
    }
    ...
    public function ajax_pagination() {
        ...
        $count = $this->getwithinbounce_model->count_pagination($filter);
        $config['div'] = 'result-pagination';
        $config['base_url'] = base_url('getwithinbounce/ajax_pagination');
        ...
        $data = array(
            'data' => $this->getwithinbounce_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
            'count' => $count,
            'segment' => $segment,
            'links' => $this->ajax_pagination->create_links()
        );
        $this->load->view('ajax/getwithinbounce_pagination', $data);
    }

    public function detail($work_process_id_pri, $check = null) {
        $workprocess = $this->getwithinbounce_model->getworkprocess($work_process_id_pri);
        if ($workprocess->num_rows() > 0) {
            $dataupdate = array(
                'work_process_receive_user_id' => $this->session->userdata('user_id'),
                'work_process_receive_name' => $this->getwithinbounce_model->ref_user(),
...
//                'work_process_sendstatus' => 0,
            ...
            $this->getwithinbounce_model->update_workprocess($work_process_id_pri, $dataupdate);
            $data = array(
                'group_id' => $this->group_id,
                'menu_id' => $this->menu_id,
                'icon' => $this->accesscontrol->getIcon($this->group_id),
                ...
                'data' => $this->getwithinbounce_model->getworkinfo($workprocess->row()->work_info_id_pri)->row(),
            );
            if ($check != null) {
                $text = 'เปิด หนังสือส่งไปยังหน่วยงาน';
                $this->systemlog->log_work_process($text, $work_process_id_pri);
            }
            $this->renderView('getwithinbouncedetail_view', $data);
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('getwithinbounce'));
        }
    }

    public function add_comment() {
        $work_process_id_pri = $this->input->post('work_process_id_pri');
        $workprocess = $this->getwithinbounce_model->checkworkprocess($work_process_id_pri);
        if ($workprocess->num_rows() == 1) {
            ...
            if ($row->work_process_receive == 1) {
                ...
                $this->getwithinbounce_model->update_workprocess($work_process_id_pri, $data);
                $this->session->set_flashdata('flash_message', 'success,สำเร็จ,ทำรายการสำเร็จแล้ว');
                redirect(base_url('getwithinbounce/detail/' . $work_process_id_pri));
            }
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('getwithinbounce'));
    ...
    public function attach($work_process_id_pri = null) {
        if ($work_process_id_pri != null) {
            ...
            if ($workprocess->num_rows() > 0) {
                ...
                $this->renderView('getwithinbounceattach_view', $data);
            } else {
                $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
                redirect(base_url('getwithinbounce'));
            }
    ...
    public function ajax_page() {
        $work_process_id_pri = $this->input->post('work_process_id_pri');
        $workprocess = $this->getwithinbounce_model->getworkprocess($work_process_id_pri)->row();
        $data = array(
            'datas' => $this->getwithinbounce_model->get_workprocessfofile($workprocess->work_process_id_to),
        );
        $this->load->view('ajax/getwithinbounce_page', $data);
    }
        ...
        foreach ($_FILES as $key) {
            ...
            if ($key['size'] < 20971520) {
                ...
                if ($this->upload->do_upload($input_name)) {
                    ...
                    $file_type_id = $this->getwithinbounce_model->ref_file_type($name_type)->row()->file_type_id;
                    $workprocess = $this->getwithinbounce_model->getworkprocess($work_process_id_pri)->row();
                    ...
                    $work_process_file_id = $this->getwithinbounce_model->insert_workprocessfile($data);
                    $datalog = array(
                        'log_type_id' => 2,
                    ...
                    // $users = $this->getwithinbounce_model->getuser_dep_off($workprocess->dep_off_id);
    ...
    public function delete_file() {
        ...
        $this->systemlog->log_file($datalog);
        // $workprocess = $this->getwithinbounce_model->get_workprocessfofile_id($id)->row();
        // $users = $this->getwithinbounce_model->getuser_dep_off($workprocess->dep_off_id);
        ...
        //     }
        // }
        $this->getwithinbounce_model->delete_workprocessfile($id);
    }
    ...
    public function sendback() {
        ...
        $work_process_id_pri = $this->input->post('work_process_id_pri');
        $workinfo = $this->getwithinbounce_model->getworkinfo($work_info_id_pri);
        if ($workinfo->num_rows() == 1) {
            $row = $workinfo->row();
            $this->getwithinbounce_model->update_workprocess($work_process_id_pri, array(
                'work_process_status_id' => 1,
                'work_process_sendstatus' => 1,
                'work_process_update' => $this->misc->getdate(),
            ));
            $workproces = $this->getwithinbounce_model->getworkprocess($work_process_id_pri)->row();
            $workprocesback = $this->getwithinbounce_model->getworkprocess($workproces->work_process_id_to)->row();
            ...
            $this->systemlog->log_work_process($text, $work_process_id_pri);
            $work_process_id_pri_log = $this->getwithinbounce_model->insert_workprocess($data);
            $text = 'สร้าง(ส่งต่อ) หนังสือส่งไปยังหน่วยงาน';
            $this->systemlog->log_work_process($text, $work_process_id_pri_log);
            $users = $this->getwithinbounce_model->getuser_dep_off($dep_off_id);
            ...
            $this->getwithinbounce_model->update_workinfo($row->work_info_id_pri, array('state_info_id' => 4, 'work_info_update' => $this->misc->getdate()));
            $this->session->set_flashdata('flash_message', 'success,สำเร็จ,ทำรายการสำเร็จแล้ว');
            echo 1;
        } else {
            echo 0;
    ...
    public function modal_closestatus() {
        ...
        if ($workinfo->num_rows() == 1) {
            if ($workinfo->row()->state_info_id <= 5 || $workinfo->row()->state_info_id == 8) {
                ...
                $this->load->view('modal/getwithinbounceclosestatus_modal', $data);
            } else {
                echo 0;
            }
        } else {
    ...
    public function closestatus() {
        ...
        if ($workinfo->num_rows() == 1) {
            if ($workinfo->row()->state_info_id <= 5 || $workinfo->row()->state_info_id == 8) {
                ...
                $this->getwithinbounce_model->update_workinfo($work_info_id_pri, $data);
                echo 1;
            } else {
                echo 0;
            }
    ...
    public function attachall($work_info_code, $work_process_id_pri) {
        $workinfo = $this->getwithinbounce_model->getworkinfocode($work_info_code);
        if ($workinfo->num_rows() > 0) {
            ...
            $this->renderView('getwithinbounceattachall_view', $data);
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('getwithinbounce'));
        }
    ...
    public function ajax_pageall() {
        $work_info_id_pri = $this->input->post('work_info_id_pri');
        $data = array(
            'datas' => $this->getwithinbounce_model->get_workinfofile($work_info_id_pri),
        );
        $this->load->view('ajax/getwithinbounceall_page', $data);
    }
        ...
        foreach ($_FILES as $key) {
            ...
            if ($key['size'] < 20971520) {
                if ($this->upload->do_upload($input_name)) {
                    ...
                    $work_info_file_id = $this->getwithinbounce_model->insert_workinfofile($data);
                    $datalog = array(
                        'log_type_id' => 1,
                    ...
                    $this->systemlog->log_file($datalog);
                    $withinprocess = $this->getwithinbounce_model->get_withinprocess($work_info_id_pri);
                    foreach ($withinprocess->result() as $row) {
                        $users = $this->getwithinbounce_model->getuser_dep_off($row->dep_off_id);
    ...
    public function delete_fileall() {
        ...
        $this->systemlog->log_file($datalog);
        // $work_info_id_pri = $this->getwithinbounce_model->get_workinfofileid($id)->row()->work_info_id_pri;
        // $withinprocess = $this->getwithinbounce_model->get_withinprocess($work_info_id_pri);
        // foreach ($withinprocess->result() as $row) {
        //     $users = $this->getwithinbounce_model->getuser_dep_off($row->dep_off_id);
        ...
        //     }
        // }
        $this->getwithinbounce_model->delete_workinfofile($id);
    }
