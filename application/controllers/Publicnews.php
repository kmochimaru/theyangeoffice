...
/**
 * Description of Publicnews
 *
 * @author nut
 */
class Publicnews extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('publicnews_model');
    }

    public function index() {
        if ($this->publicnews_model->checkDepartmentyear($this->session->userdata('dep_id_pri'))->num_rows() == 1) {
            $data = array(
            ...
            $this->renderView('publicnews_view', $data);
        } else {
    ...
    public function add() {
        if ($this->input->post('work_info_no') != null) {
            $data = array(
                ...
                'secret_level_id' => 6, //$this->input->post('secret_level_id'),
            ...
            $work_info_id_pri = $this->publicnews_model->insert_workinfo($data);
            if ($work_info_id_pri > 0) {
                $dep_year = $this->publicnews_model->getDepartmentyear($this->input->post('dep_id_pri'));
                $data = array(
                    'work_info_code' => $dep_year->year . $this->input->post('dep_id_pri') . $work_info_id_pri . date('YmdHis'),
                );
                $this->publicnews_model->update_workinfo($work_info_id_pri, $data);
                $text = 'บันทึกหนังสือประชาสัมพันธ์';
