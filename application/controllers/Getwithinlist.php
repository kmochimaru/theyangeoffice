...
/**
 * Description of Getwithinlist
 *
 * @author nut
 */
class Getwithinlist extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('getwithinlist_model');
    ...
    public function index() {
        ...
        $this->renderView('getwithinlist_view', $data);
    }
    ...
    public function ajax_pagination() {
        ...
        $count = $this->getwithinlist_model->count_pagination($filter);
        $config['div'] = 'result-pagination';
        $config['base_url'] = base_url('getwithinlist/ajax_pagination');
        ...
        $data = array(
            'data' => $this->getwithinlist_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
            'count' => $count,
            'segment' => $segment,
            'links' => $this->ajax_pagination->create_links()
        );
        $this->load->view('ajax/getwithinlist_pagination', $data);
    }

    public function detail($work_info_code) {
        $workinfo = $this->getwithinlist_model->getworkinfocode($work_info_code);
    ...
    public function changestatus() {
        ...
        if ($workinfo->num_rows() == 1) {
            if ($workinfo->row()->state_info_id < 5) {
                ...
                $this->getwithinlist_model->update_workinfo($work_info_id_pri, $data);
                echo 1;
            } else {
                echo 0;
            }
    ...
    public function attach($work_info_code) {
        ...
        if ($workinfo->num_rows() > 0) {
            ...
            $this->renderView('getwithinlistattach_view', $data);
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('getwithinlist'));
        }
    ...
    public function ajax_page() {
        $work_info_id_pri = $this->input->post('work_info_id_pri');
        $data = array(
            'datas' => $this->getwithinlist_model->get_workinfofile($work_info_id_pri),
        );
        $this->load->view('ajax/getwithinlist_page', $data);
    }
        ...
        foreach ($_FILES as $key) {
            ...
            if ($key['size'] < 20971520) {
                if ($this->upload->do_upload($input_name)) {
                    ...
                    $file_type_id = $this->getwithinlist_model->ref_file_type($name_type)->row()->file_type_id;
                    $data = array(
                    ...
                    $work_info_file_id = $this->getwithinlist_model->insert_workinfofile($data);
                    $datalog = array(
                        'log_type_id' => 1,
                    ...
                    $withinprocess = $this->getwithinlist_model->get_withinprocess($work_info_id_pri);
                    foreach ($withinprocess->result() as $row) {
                        $users = $this->getwithinlist_model->getuser_dep_off($row->dep_off_id);
    ...
    public function delete_file() {
        ...
        $this->systemlog->log_file($datalog);
        $work_info_id_pri = $this->getwithinlist_model->get_workinfofileid($id)->row()->work_info_id_pri;
        $withinprocess = $this->getwithinlist_model->get_withinprocess($work_info_id_pri);
        foreach ($withinprocess->result() as $row) {
            $users = $this->getwithinlist_model->getuser_dep_off($row->dep_off_id);
        ...
        $this->getwithinlist_model->delete_workinfofile($id);
    }
