...
class Getwithin extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('getwithin_model');
    }

    public function index() {
        if ($this->getwithin_model->checkDepartmentyear($this->session->userdata('dep_id_pri'))->num_rows() == 1) {
            $data = array(
            ...
            $this->renderView('getwithin_view', $data);
        } else {
    ...
    public function add() {
        $dep_years = $this->getwithin_model->checkDepartmentyear($this->session->userdata('dep_id_pri'));
        if ($dep_years->num_rows() > 0) {
            ...
            if ($this->input->post('work_info_no') != null) {
                ...
                $work_info_id_pri = $this->getwithin_model->insert_workinfo($data);
                if ($work_info_id_pri > 0) {
                    $dep_year = $this->getwithin_model->getDepartmentyear($this->input->post('dep_id_pri'));
                    $data = array(
                        'work_info_code' => $dep_year->year . $this->input->post('dep_id_pri') . $work_info_id_pri . date('YmdHis'),
                        'work_info_id' => $dep_year->dep_year_receive_last,
                    );
                    $this->getwithin_model->update_workinfo($work_info_id_pri, $data);
                    $this->getwithin_model->update_departmentyear(array('dep_year_receive_last' => $dep_year->dep_year_receive_last += 1, 'dep_year_update' => $this->misc->getdate()));
                    $text = 'ลงทะเบียน' . $this->getwithin_model->ref_work_type($this->input->post('work_type_id'))->row()->work_type_name;
                    ...
                    $dataprocess = array(
                        ...
                        'special_command_id' => 2,
                        'work_process_receive_user_id' => $this->session->userdata('user_id'),
                        'work_process_receive_name' => $this->getwithin_model->ref_user(),
                        'work_process_receive_id' => $workinfo->work_info_id,
                        ...
                        'work_process_sort' => 1,
                        'work_process_active_id' => 0,
                        'work_process_create' => $this->misc->getdate(),
                        'work_process_update' => $this->misc->getdate(),
                    );
                    $work_process_id_pri = $this->getwithin_model->insert_workprocess($dataprocess);
                    $text = 'สร้าง หนังสือส่งไปยังหน่วยงาน (หนังสือรับ)';
    ...
    public function ajax_page() {
        ...
        $data = array(
            'datas' => $this->getwithin_model->get_workinfofile($work_info_id_pri),
        );
        $this->load->view('ajax/getwithin_page', $data);
    }

    public function ajax_detail() {
        $work_info_id_pri = $this->input->post('work_info_id_pri');
        $workinfo = $this->getwithin_model->getWorkinfoByID($work_info_id_pri);
    ...
    public function from_modal() {
        $this->load->view('modal/getwithin_from_modal');
    }

    public function check_info_number() {
        $filter = array(
            'year_id' => $this->session->userdata('year_id'),
            'work_info_no' => $this->input->post('work_info_no')
        );
        echo $this->getwithin_model->check_info_number($filter);
    }
}
