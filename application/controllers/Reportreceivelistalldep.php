...
class Reportreceivelistalldep extends CI_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model('reportreceivelistalldep_model');
  ...
  public function index()
  {
    ...
    $this->renderView('reportreceivelistalldep_view', $data);
  }
...
//      'work_type_id' => $this->input->post('work_type_id'),
    ...
    $count = $this->reportreceivelistalldep_model->count_pagination($filter);
    ...
    $data = array(
      'data' => $this->reportreceivelistalldep_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
      'count' => $count,
      'segment' => $segment,
      'links' => $this->ajax_pagination->create_links()
    );
    $this->load->view('ajax/reportreceivelistalldep_pagination', $data);
  }
  ...
  public function excel($year = null, $searchtext = null, $status_id = null, $book_group_id = null, $dateStart = null, $dateEnd = null)
  {
    ...
    $data = $this->reportreceivelistalldep_model->get_dataexcel($filter);
    ...
    if ($data->num_rows() > 0) {
      foreach ($data->result() as $rows) {
        ...
        if ($row->work_process_sendtype != 2) {
          ...
          $sheet->setCellValue("H$r", $this->reportreceivelistalldep_model->getdep_off($row->work_info_dep_id_pri)->row()->dep_name);
          $sheet->setCellValue("I$r", $this->reportreceivelistalldep_model->getdep_off_id($row->work_info_dep_off_id)->officer_name);
          ...
          $sheet->setCellValue("P$r", $this->reportreceivelistalldep_model->getdep_off_id($row->dep_off_id)->officer_name);
