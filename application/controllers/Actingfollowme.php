...
/**
 * Description of Actingfollowme
 *
 * @author nut
 */
class Actingfollowme extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('actingfollowme_model');
    ...
    public function index() {
        ...
        $this->renderView('actingfollowme_view', $data);
    }
    ...
    public function ajax_pagination() {
        ...
        $count = $this->actingfollowme_model->count_pagination($filter);
        $config['div'] = 'result-pagination';
        $config['base_url'] = base_url('actingfollowme/ajax_pagination');
        ...
        $data = array(
            'data' => $this->actingfollowme_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
            'count' => $count,
            'segment' => $segment,
            'links' => $this->ajax_pagination->create_links()
        );
        $this->load->view('ajax/actingfollowme_pagination', $data);
    }

    public function detail($work_info_code) {
        $workinfo = $this->actingfollowme_model->getworkinfocode($work_info_code);
    ...
    public function unfollow() {
        ...
        $workinfo = $this->actingfollowme_model->getworkinfo($work_info_id_pri);
        if ($workinfo->num_rows() == 1) {
            if ($workinfo->row()->work_info_follow == 1) {
                ...
                $this->actingfollowme_model->update_workinfo($work_info_id_pri, $data);
                echo 1;
            } else {
                echo 0;
            }
