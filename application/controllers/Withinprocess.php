...
/**
 * Description of Withinprocess
 *
 * @author nut
 */
class Withinprocess extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('withinprocess_model');
    ...
    public function index() {
        ...
        $this->renderView('withinprocess_view', $data);
    }
    ...
    public function ajax_pagination() {
        ...
        $count = $this->withinprocess_model->count_pagination($filter);
        $config['div'] = 'result-pagination';
        $config['base_url'] = base_url('withinprocess/ajax_pagination');
        ...
        $data = array(
            'data' => $this->withinprocess_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
            'count' => $count,
            'segment' => $segment,
            'links' => $this->ajax_pagination->create_links()
        );
        $this->load->view('ajax/withinprocess_pagination', $data);
    }

    public function detail($work_process_id_pri = null) {
        if ($work_process_id_pri != null) {
            $workprocess = $this->withinprocess_model->getworkprocess($work_process_id_pri);
            if ($workprocess->num_rows() > 0) {
                ...
                $data = array(
                    ...
                    'data' => $this->withinprocess_model->getworkinfo($row->work_info_id_pri)->row(),
                );
                $this->renderView('withinprocessdetail_view', $data);
            } else {
                $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
                redirect(base_url('withinprocess'));
            }
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('withinprocess'));
    ...
    public function modal_changestatus() {
        ...
        if ($workprocess->num_rows() == 1) {
            if ($workprocess->row()->work_process_receive == 0) {
                ...
                $this->load->view('modal/withinprocesschangestatus_modal', $data);
            } else {
                echo 0;
            }
        } else {
    ...
    public function changestatus() {
        ...
        if ($workprocess->num_rows() == 1) {
            if ($workprocess->row()->work_process_receive == 0) {
                ...
                $this->withinprocess_model->update_workprocess($work_process_id_pri, $data);
                $text = 'ดึงเรื่องกลับ หนังสือส่งไปยังหน่วยงาน';
                $this->systemlog->log_work_process($text, $work_process_id_pri);
                $users = $this->withinprocess_model->getuser_dep_off($workprocess->row()->dep_off_id);
                ...
                $workinfo = $this->withinprocess_model->getworkinfoprocess($work_info_id_pri);
                if ($workinfo->num_rows() == $this->withinprocess_model->checkworkinfo7($work_info_id_pri)) {
                    $data = array(
                        'state_info_id' => 7,
                        'work_info_update' => $this->misc->getdate(),
                    );
                    $this->withinprocess_model->update_workinfo($workprocess->row()->work_info_id_pri, $data);
        ...
        if ($work_process_id_pri != null) {
            ...
            if ($workprocess->num_rows() > 0) {
                $data = array(
                    ...
                    'work_process_id_pri' => $work_process_id_pri,
                    'work_info_id_pri' => $workprocess->row()->work_info_id_pri,
                    'data' => $this->withinprocess_model->getworkinfo($workprocess->row()->work_info_id_pri)->row(),
                );
                $this->renderView('withinprocessattach_view', $data);
            } else {
                $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
                redirect(base_url('withinprocess'));
            }
    ...
    public function ajax_page() {
        $work_process_id_pri = $this->input->post('work_process_id_pri');
        $data = array(
            'datas' => $this->withinprocess_model->get_workprocessfofile($work_process_id_pri),
        );
        $this->load->view('ajax/withinprocess_page', $data);
    }
        ...
        foreach ($_FILES as $key) {
            ...
            if ($key['size'] < 20971520) {
                ...
                if ($this->upload->do_upload($input_name)) {
                    ...
                    $file_type_id = $this->withinprocess_model->ref_file_type($name_type)->row()->file_type_id;
                    ...
                    $work_process_file_id = $this->withinprocess_model->insert_workprocessfile($data);
                    $datalog = array(
                        'log_type_id' => 2,
                    ...
                    // $workprocess = $this->withinprocess_model->getworkprocess($work_process_id_pri)->row();
                    // $users = $this->withinprocess_model->getuser_dep_off($workprocess->dep_off_id);
    ...
    public function delete_file() {
        ...
        $this->systemlog->log_file($datalog);
        // $workprocess = $this->withinprocess_model->get_workprocessfofile_id($id)->row();
        // $users = $this->withinprocess_model->getuser_dep_off($workprocess->dep_off_id);
        ...
        //     }
        // }
        $this->withinprocess_model->delete_workprocessfile($id);
    }
