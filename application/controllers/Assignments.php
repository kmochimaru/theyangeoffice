...
class Assignments extends CI_Controller {
    ...
    public function index() {
        ...
        $this->renderView('assignments_view', $data);
    }
    ...
    public function ajax_pagination() {
        ...
        $count = $this->assignments_model->count_pagination($filter);
        $config['div'] = 'result-pagination';
        $config['base_url'] = base_url('assignments/ajax_pagination');
        ...
        $data = array(
            'data' => $this->assignments_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
            'count' => $count,
            'segment' => $segment,
            'links' => $this->ajax_pagination->create_links()
        );
        $this->load->view('ajax/assignments_pagination', $data);
    }

    public function detail($work_user_id) {
        $workusers = $this->assignments_model->getworkuser($work_user_id);
        if ($workusers->num_rows() > 0) {
            $data = array(
                ...
                'data' => $this->assignments_model->getworkinfo($workusers->row()->work_info_id_pri)->row(),
            );
            $this->renderView('assignmentsdetail_view', $data);
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('assignments'));
        }
    }
}
