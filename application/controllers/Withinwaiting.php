...
class Withinwaiting extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('withinwaiting_model');
    ...
    public function index() {
        ...
        $this->renderView('withinwaiting_view', $data);
    }
    ...
    public function ajax_pagination() {
        ...
        $count = $this->withinwaiting_model->count_pagination($filter);
        $config['div'] = 'result-pagination';
        $config['base_url'] = base_url('withinwaiting/ajax_pagination');
        ...
        $data = array(
            'data' => $this->withinwaiting_model->get_pagination($filter, array('start' => $segment, 'limit' => $this->per_page)),
            'count' => $count,
            'segment' => $segment,
            'links' => $this->ajax_pagination->create_links()
        );
        $this->load->view('ajax/withinwaiting_pagination', $data);
    }

    public function edit($work_info_id_pri = null) {
        if ($work_info_id_pri != null) {
            $workinfo = $this->withinwaiting_model->getworkinfo($work_info_id_pri);
            if ($workinfo->num_rows() > 0) {
                ...
                $this->renderView('withinedit_view', $data);
            } else {
        ...
        if ($work_info_id_pri != null || $work_info_id_pri != '') {
            ...
            if ($workinfo->num_rows() > 0) {
                $data = array(
                    ...
                    //'internal_action_name' => $this->input->post('internal_action_name'),
                    'attach_original' => $this->input->post('attach_original'),
                    'work_info_update' => $this->misc->getdate()
                );
                $this->withinwaiting_model->update_workinfo($work_info_id_pri, $data);
                $text = 'แก้ไขหนังสือส่งภายใน';
...
//                    'work_info_no_3' => $workinfo->work_info_no_3,
                ...
                redirect(base_url('withinwaiting/edit/' . $work_info_id_pri));
            } else {
                $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
                redirect(base_url('withinwaiting'));
            }
    ...
    public function attach($work_info_id_pri) {
        if ($work_info_id_pri != null) {
            ...
            if ($workinfo->num_rows() > 0) {
                ...
                $this->renderView('withinattach_view', $data);
            } else {
                $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
                redirect(base_url('withinwaiting'));
            }
    ...
    public function from_modal() {
        $this->load->view('modal/withinwaiting_from_modal');
    }

    public function to_modal() {
        $this->load->view('modal/withinwaiting_to_modal');
    }
    ...
    public function ajax_page() {
        ...
        $workinfo = $this->withinwaiting_model->getworkinfo($work_info_id_pri);
        $data = array(
            'datas' => $this->withinwaiting_model->get_workinfofile($work_info_id_pri),
            'state_info_id' => $workinfo->row()->state_info_id,
        );
        $this->load->view('ajax/withinwaiting_page', $data);
    }
        ...
        foreach ($_FILES as $key) {
            ...
            if ($key['size'] < 20971520) {
                if ($this->upload->do_upload($input_name)) {
                    ...
                    $file_type_id = $this->withinwaiting_model->ref_file_type($name_type)->row()->file_type_id;
                    $data = array(
                        'work_info_id_pri' => $work_info_id_pri,
                    ...
                    $work_info_file_id = $this->withinwaiting_model->insert_workinfofile($data);
                    $datalog = array(
                        'log_type_id' => 1,
                    ...
                    $this->withinwaiting_model->update_workinfo($work_info_id_pri, array('state_info_id' => 3, 'work_info_update' => $this->misc->getdate()));
                    $status[$i] = 1;
                } else {
                    $status[$i] = 2;
                }
    ...
    public function notupload_attach($work_info_id_pri) {
        ...
        if ($workinfo->num_rows() == 1) {
            $row = $workinfo->row();
            $files = $this->withinwaiting_model->get_workinfofile($work_info_id_pri);
            if ($files->num_rows() == 0) {
    ...
    public function delete_file() {
        ...
        $workinfofile = $this->withinwaiting_model->get_workinfofileid($id)->row();
        ...
        $this->systemlog->log_file($datalog);
        $this->withinwaiting_model->delete_workinfofile($id);
        if ($this->withinwaiting_model->get_workinfofile($workinfofile->work_info_id_pri)->num_rows() == 0) {
            $this->withinwaiting_model->update_workinfo($workinfofile->work_info_id_pri, array('state_info_id' => 2, 'work_info_update' => $this->misc->getdate()));
        }
    }
    ...
    //     $work_info_id_pri = $this->input->post('work_info_id_pri');
    //     $data = array(
    //         'data' => $this->withinwaiting_model->getworkinfo($work_info_id_pri)->row(),
    //     );
    ...
    public function send($work_info_id_pri) {
        ...
        if ($workinfo->num_rows() > 0) {
            ...
            $this->renderView('send_view', $data);
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('withinwaiting'));
        }
    ...
    public function sendsort($work_info_id_pri) {
        ...
        if ($workinfo->num_rows() > 0) {
            ...
            $this->renderView('sendsort_view', $data);
        } else {
            $this->session->set_flashdata('flash_message', 'error,เกิดข้อผิดผลาด,ไม่สามารถทำรายการได้');
            redirect(base_url('withinwaiting'));
        }
        ...
        if ($workinfo->num_rows() == 1) {
            ...
            if (count($select_checkbox) != 0) {
                ...
                foreach ($select_checkbox as $dep_off_id) {
                    ...
                    $work_process_id_pri = $this->withinwaiting_model->insert_workprocess($data);
                    $text = 'สร้าง หนังสือส่งไปยังหน่วยงาน';
                    $this->systemlog->log_work_process($text, $work_process_id_pri);
                    //print_r($data);
                    $users = $this->withinwaiting_model->getuser_dep_off($dep_off_id);
                ...
                $text = 'ส่งหนังสือส่งภายใน';
                $this->systemlog->log_work_info($text, $work_info_id_pri);
                $this->withinwaiting_model->update_workinfo($row->work_info_id_pri, array('state_info_id' => 4, 'work_info_update' => $this->misc->getdate()));
                $this->session->set_flashdata('flash_message', 'success,สำเร็จ,ทำรายการสำเร็จแล้ว');
                echo 1;
            } else {
                echo 0;
    ...
    public function follow() {
        ...
        $workinfo = $this->withinwaiting_model->getworkinfo($work_info_id_pri);
        if ($workinfo->num_rows() == 1) {
            $this->withinwaiting_model->update_workinfo($this->input->post('work_info_id_pri'), array('work_info_follow' => $this->input->post('work_info_follow'), 'work_info_update' => $this->misc->getdate()));
            echo 1;
        } else {
            echo 0;
        }
    ...
    public function attach_original() {
        ...
        $workinfo = $this->withinwaiting_model->getworkinfo($work_info_id_pri);
        if ($workinfo->num_rows() == 1) {
            $this->withinwaiting_model->update_workinfo($this->input->post('work_info_id_pri'), array('attach_original' => $this->input->post('attach_original'), 'work_info_update' => $this->misc->getdate()));
            echo 1;
        } else {
            echo 0;
        }
    ...
    //                 'row' => $workinfo->row(),
    //             );
    //             $this->load->view('modal/withinregisnumber_modal', $data);
    ...
    //     if ($workinfo->num_rows() == 1) {
    //         if ($workinfo->row()->state_info_id == 2) {
    //             $dep_years = $this->withinwaiting_model->getDepartmentyear($this->session->userdata('dep_id_pri'));
    ...
    //                 );
    //                 $this->withinwaiting_model->update_workinfo($work_info_id_pri, $data);
    //                 $this->withinwaiting_model->update_departmentyear(array('dep_year_send_last' => $dep_year->dep_year_send_last += 1, 'dep_year_update' => $this->misc->getdate()));
